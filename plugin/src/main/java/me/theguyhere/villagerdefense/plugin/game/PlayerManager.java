...
public class PlayerManager {
	...
	public static void giveItem(Player player, ItemStack item, String message) {
		...
		if (player
			...
			.firstEmpty() == -1 && (player
			.getInventory()
			.first(item.getType()) == -1 ||
			(player
				.getInventory()
				.all(new ItemStack(item.getType(), item.getMaxStackSize()))
				.size() ==
				player
					.getInventory()
					.all(item.getType())
	...
	public static void namedNotify(
		Player player, ColoredMessage name, ColoredMessage base,
	...
	public static int getCrystalBalance(UUID id) {
		if (Main.hasCustomEconomy())
			return (int) Main
				.getEconomy()
				.getBalance(Bukkit.getOfflinePlayer(id));
		else return Main
			.getPlayerData()
			.getInt(id + ".crystalBalance");
	}
	...
	public static void depositCrystalBalance(UUID id, int amount) {
		if (Main.hasCustomEconomy())
			Main
				.getEconomy()
				.depositPlayer(Bukkit.getOfflinePlayer(id), amount);
		else Main
			.getPlayerData()
			.set(id + ".crystalBalance", getCrystalBalance(id) + amount);
		Main.savePlayerData();
	}
	...
	public static void withdrawCrystalBalance(UUID id, int amount) {
		if (Main.hasCustomEconomy())
			Main
				.getEconomy()
				.withdrawPlayer(Bukkit.getOfflinePlayer(id), amount);
		else Main
			.getPlayerData()
			.set(id + ".crystalBalance", Math.max(getCrystalBalance(id) - amount, 0));
		Main.savePlayerData();
	}
	...
	public static void addAchievement(UUID id, String achievementID) {
		...
		List<String> achievements = Main
			.getPlayerData()
			.getStringList(path);
		achievements.add(achievementID);
		Main
			.getPlayerData()
			.set(path, achievements);
		Main.savePlayerData();
	}
	...
	public static void cacheSurvivalStats(Player player) {
		...
		Main
			.getPlayerData()
			.set(id + ".health", player.getHealth());
		Main
			.getPlayerData()
			.set(id + ".food", player.getFoodLevel());
		Main
			.getPlayerData()
			.set(id + ".saturation", (double) player.getSaturation());
		Main
			.getPlayerData()
			.set(id + ".level", player.getLevel());
		Main
			.getPlayerData()
			.set(id + ".exp", (double) player.getExp());
		for (int i = 0; i < player
			.getInventory()
			.getContents().length; i++)
			Main
				.getPlayerData()
				.set(
					id + ".inventory." + i,
	...
	public static void returnSurvivalStats(Player player) {
		...
		if (Main
			.getPlayerData()
			.contains(id + ".mode"))
			player.setGameMode(GameMode.valueOf(Main
				.getPlayerData()
				.getString(id + ".mode")));
		Main
			.getPlayerData()
		...
		if (Main
			.getPlayerData()
			.contains(id + ".saturation"))
			player.setSaturation((float) Main
				.getPlayerData()
				.getDouble(id + ".saturation"));
		Main
			.getPlayerData()
			.set(id + ".saturation", null);
		if (Main
			.getPlayerData()
			.contains(id + ".level"))
			...
			Objects
				...
				.forEach(num -> player
					...
					.setItem(
						...
						(ItemStack) Main
							.getPlayerData()
							.get(id + ".inventory." + num)
					));
		Main
			.getPlayerData()
			.set(id + ".inventory", null);
		Main.savePlayerData();
	}
